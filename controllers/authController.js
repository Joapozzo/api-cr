    ...
    (err, result) => {
      ...
      if (!bandera) {
        ...
        if (result.length > 0 && result[0].count > 0) {
          ...
          return res
            .status(400)
            .send("Correo electrónico no encontrado en la base de datos");
        }
      }
    }
  );
...
const checkDni = (req, res) => {
  const { dni } = req.body;
  db.query(
    "SELECT COUNT(*) AS count FROM usuarios WHERE dni = ?",
    [dni],
    (err, result) => {
          ...
          if (email) {
            try {
              ...
              return res
                ...
                .json({
                  message:
                    "Hubo un error en el envío del mail de autenticación",
                });
            }
          }
...
const checkLogin = (req, res) => {
  ...
  db.query("SELECT * FROM usuarios WHERE dni = ?", [dni], (err, rows) => {
    ...
    if (!bcryptjs.compareSync(password, user.clave))
      return res.status(405).send("Contraseña incorrecta");
    ...
    res
      .status(200)
      .json({ token, id_rol: user.id_rol, id_user: user.id_usuario });
  });
};
...
const checkAuthentication = async (req, res) => {
  try {
    ...
    const result = await query("SELECT * FROM usuarios WHERE dni = ?", [
      decoded.user,
    ]);
    ...
    console.error("❌ Error en checkAuthentication:", error.message);
...
const forgotPasswordHandler = async (req, res) => {
  ...
  try {
    db.query(
      "SELECT dni, email FROM usuarios WHERE email = ?",
      [email],
      async (err, result) => {
        if (err) {
        ...
        const resetToken = jsonwebtoken.sign({ dni }, "your-secret-key", {
          expiresIn: "3m",
        });

        db.query(
          "UPDATE usuarios SET reset_token = ?, reset_token_expiration = ? WHERE dni = ?",
          ...
          async (err) => {
            ...
            try {
              ...
              return res
                .status(200)
                .send("Mail de recuperación enviado con éxito");
            } catch (mailError) {
              console.error("Error al enviar el correo:", mailError);
              return res
                .status(500)
                .json({
                  message: "Hubo un error en el envío del mail de recuperación",
                });
            }
          }
        );
    ...
    res
      .status(500)
      .json({ message: "Hubo un error en el envío del mail de recuperación" });
  }
};
...
const changeNewPassword = (req, res) => {
  ...
  jsonwebtoken.verify(token, "your-secret-key", (err, decoded) => {
    ...
    db.query(
      "SELECT reset_token_expiration FROM usuarios WHERE dni = ?",
      [dni],
      (err, results) => {
        if (err) {
        ...
        bcryptjs.genSalt(10, (err, salt) => {
          ...
          bcryptjs.hash(clave, salt, (err, hash) => {
            ...
            db.query(
              "UPDATE usuarios SET clave = ?, reset_token = NULL, reset_token_expiration = NULL WHERE dni = ?",
              [hash, dni],
              (err, results) => {
                if (err) {
                  console.error(
